	...
	public class HelpInfo {
		...
		public static void FillTable() {
			...
			for (int i = 0; i < commands.Count; ++i) {
				...
				if (usage == null || desc == null) continue;
				...
				if (aliases == null) list.Add(new CommandInfo(e.AccessLevel, e.Command, null, usage.Usage, descString));
				else {
			...
			for (int i = 0; i < TargetCommands.AllCommands.Count; ++i) {
				...
				if (command.Supports != CommandSupport.Single) {
					...
					if ((command.Supports & CommandSupport.Global) != 0) sb.Append("<i>Global</i>, ");

					if ((command.Supports & CommandSupport.Online) != 0) sb.Append("<i>Online</i>, ");

					if ((command.Supports & CommandSupport.Region) != 0) sb.Append("<i>Region</i>, ");

					if ((command.Supports & CommandSupport.Contained) != 0) sb.Append("<i>Contained</i>, ");

					if ((command.Supports & CommandSupport.Multi) != 0) sb.Append("<i>Multi</i>, ");

					if ((command.Supports & CommandSupport.Area) != 0) sb.Append("<i>Area</i>, ");

					if ((command.Supports & CommandSupport.Self) != 0) sb.Append("<i>Self</i>, ");
			...
			foreach(CommandInfo c in m_SortedHelpInfo) {
				if (!m_HelpInfos.ContainsKey(c.Name.ToLower())) m_HelpInfos.Add(c.Name.ToLower(), c);
			}
		}
		...
		private static void HelpInfo_OnCommand(CommandEventArgs e) {
			if (e.Length > 0) {
				...
				if (m_HelpInfos.TryGetValue(arg, out c)) {
					...
					if (m.AccessLevel >= c.AccessLevel) m.SendGump(new CommandInfoGump(c));
					else m.SendMessage("You don't have access to that command.");

					return;
				} else e.Mobile.SendMessage(String.Format("Command '{0}' not found!", arg));
			}
		...
		public class CommandListGump: BaseGridGump {
			...
			public CommandListGump(int page, Mobile from, List < CommandInfo > list): base(30, 30) {
				...
				if (list == null) {
					...
					foreach(CommandInfo c in m_SortedHelpInfo) {
						if (from.AccessLevel >= c.AccessLevel) this.m_List.Add(c);
					}
				} else this.m_List = list;

				this.AddNewPage();

				if (this.m_Page > 0) this.AddEntryButton(20, ArrowLeftID1, ArrowLeftID2, 1, ArrowLeftWidth, ArrowLeftHeight);
				else this.AddEntryHeader(20);
				...
				if ((this.m_Page + 1) * EntriesPerPage < this.m_List.Count) this.AddEntryButton(20, ArrowRightID1, ArrowRightID2, 2, ArrowRightWidth, ArrowRightHeight);
				else this.AddEntryHeader(20);
			...
			public override void OnResponse(NetState sender, RelayInfo info) {
				...
				switch (info.ButtonID) {
					...
					case 1:
						{
							if (this.m_Page > 0) m.SendGump(new CommandListGump(this.m_Page - 1, m, this.m_List));

							break;
						}
					case 2:
						{
							if ((this.m_Page + 1) * EntriesPerPage < m_SortedHelpInfo.Count) m.SendGump(new CommandListGump(this.m_Page + 1, m, this.m_List));
		...
		public class CommandInfoGump: Gump {
			...
			public CommandInfoGump(CommandInfo info): this(info, 320, 210) {}

			public CommandInfoGump(CommandInfo info, int width, int height): base(300, 50) {
				m_Info = info;
				this.AddPage(0);
