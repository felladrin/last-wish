			...
			foreach( Mobile m in ie )
			{
				if( Players.IndexOf( m ) > -1 )
				{
					if( m != sourcebag.Owner )
					{
						...
						BombBag bag = (BombBag)m.Backpack.FindItemByType( typeof( BombBag ) );//Opticon
						bag.Delete();//Opticon, delete bombbag immediately after player gets killed
						...
						//handle scoring
						if(_SaveScore == true) //save score if true, Opticon
						{
					...
					else
					{
						...
						sourcebag.Delete(); //delete owners bomb
					}
					if(_SaveScore == true)
