...
namespace Joeku.MOTD
{
    public class MOTD_Utility
	{
		...
		{
			if( CheckLogin( e.Mobile.Account ) )
				SendGump( e.Mobile );
		}

		public static bool CheckLogin( IAccount iacc )
		{
			Account acc = iacc as Account;

			if( acc.GetTag( "MOTD" ) != null )
				return false;

			return true;
		}

		public static void ToggleLogin( IAccount iacc )
		{
			Account acc = iacc as Account;

			if( CheckLogin( acc ) )
				acc.SetTag( "MOTD", "This user has chosen not to display the MOTD upon login." );
			else
				acc.RemoveTag( "MOTD" );
		}
		...
		public static void SendGump( Mobile mob ){ SendGump( mob, false, 0 ); }
		public static void SendGump( Mobile mob, bool help ){ SendGump( mob, help, 0 ); }
		public static void SendGump( Mobile mob, bool help, int index )
		{
			if( !help )
				CheckFiles();

			mob.CloseGump( typeof( MOTD_Gump ) );
			mob.SendGump( new MOTD_Gump( mob, help, index ) );
		}
