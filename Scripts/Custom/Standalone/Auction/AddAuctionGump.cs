    ...
    class AddAuctionGump : BaseGridGump
    {
        ...
        public override void OnResponse(Network.NetState sender, RelayInfo info)
        {
            if (info.ButtonID == 1)
            {
                ...
                if ((String.IsNullOrEmpty(info.GetTextEntry(0).Text)) || (info.GetTextEntry(0).Text.Length < 3))
                {
                ...
                if ( (String.IsNullOrEmpty(info.GetTextEntry(1).Text)) || (info.GetTextEntry(1).Text.Length < 3))
                {
        ...
        void Accepted(Mobile from, bool ok, object o)
        {
            ...
            if (ok)
            {
                if (oh.item.Parent != oh.mob.Backpack)
                {
